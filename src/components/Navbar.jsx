        ...
        <header className='w-full bg-white md:bg-transparent fixed top-0 left-0 right-0'>
            <nav className={'py-4 lg:px-14 ${isSticky ? "sticky top-0 left-0 right-0 border bg-white duration-300" : ""}'}>
                <div className='flex justify-between items-center text-base gap-8'>
                    <a href="" className='text-2x1 font-sans flex items-center space-x-3'> <img src="" alt="logo" className='w-10 inline-block items-center' />
                        <span className='text-[#263238]'> Dr Doctor</span></a>
                    ...
                    {/* menu btn for only responsive design */}
                ...
                <div className={'space-y-4 px-4 mt-16 py-7 bg-brandPrimary ${ isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}'}>
                    {
                        navItems.map(({ link, path }) => <Link to={path} spy={true} smooth={true} offset={-200} key={path} className='block text-base text-white hover:text-brandPrimary first:font-medium'> {link}</Link>)
                    }
                </div>
            </nav>
        </header>
